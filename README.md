...
- At point t, OPTFF chooses to evict the cache that is called the furthest in the future, and (A) chooses to evict a cache that is called earlier
- We can transform S_A into S_FF at point t by making (A) evict the same cache that OPTFF does
- Evicting a cache that is called at the furthest possible time can never increase the number of cache misses
