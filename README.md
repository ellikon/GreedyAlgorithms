...
- Assume (A) is an optimal algorithm
- Let S_A be the sequence of caches generated by (A), and S_FF be the sequence generated by OPTFF
- Assume S_A and S_FF are equal up until a point **t** where they differ
- Because S_A and S_FF have been equal up until **t**, their number of misses must be equal
- At point t, let OPTFF evict **x** and (A) evict **y**
- OPTFF chooses to evict the request that is called the furthest in the future, and (A) chooses to evict a request that is called earlier or up to the furthest in the future
- This means **x** is at least as far into the future as **y**
- The algorithms maintain the same behavior until **x** or **y** is called again
- If **y** is called first then S_FF performs at least as well as S_A since S_FF kept **y** instead of evicting it
- Since **x** cannot be called before **y** the only other option is if neither is called again which would mean evicting **x** is no worse than evicting **y** 
- Repeat this transform for all points where S_A and S_FF differ
- Any optimal algorithm (A) can be transformed into algorithm OPTFF without increasing the number of misses
- Therefore, OPTFF has no more misses than (A) and is optimal
